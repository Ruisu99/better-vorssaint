...
- **Quick AI.** Optional and off until you install it. With the Command Bar open, Tab (or ` or /, your choice) switches it into a fast OpenAI chat using your own API key. Follow-ups stay in the same thread. Keep a chat to reopen it later in its own window. Optional web search uses OpenAI's search, only when you switch it on. The key stays on this Mac, owner-only, and is never part of a settings backup. Nothing is sent until you press Return.
...
Vorssaint is local-first, with no account, analytics or tracking. The network is touched only by things you can see: update checks, the speed test, Homebrew actions, temporary screenshot or recording links, feedback you explicitly send, and Quick AI when you install it and press Return with your own OpenAI key. The full story is in the [privacy notes](docs/PRIVACY.md).
